...
async function main() {
  ...
  // Find sheetId for Registro Calls
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  const sheet = meta.data.sheets?.find((s) => s.properties?.title?.includes("Registro Calls"));
  const sheetId = sheet?.properties?.sheetId;
  if (sheetId == null) throw new Error("Sheet not found");
  console.log(`sheetId: ${sheetId}, current rows: ${sheet?.properties?.gridProperties?.rowCount}`);

  // Add 20 rows to the grid
  ...
  console.log("Extended grid by 20 rows");
  ...
  for (const lead of leads) {
    if (!lead.sheets_row_index) {
      console.log(`  ${lead.nombre} has no row yet, skipping`);
      continue;
    }
