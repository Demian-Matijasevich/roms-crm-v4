...
export async function POST(req: NextRequest) {
  ...
  const campaigns = [
    {
      url: "https://app.iclosed.io/e/romsconsultora/s-llamada-de-consultoria-i-roms-consultora?utm_source=inbound&utm_medium=guille&utm_content=guille",
      source: "inbound",
      medium: "guille",
      content: "guille",
    },
    {
      url: "https://app.iclosed.io/e/romsconsultora/ot-llamada-de-consultoria-i-roms-consultora?utm_source=outbound&utm_medium=guille&utm_content=guille",
      source: "outbound",
      medium: "guille",
      content: "guille",
    },
  ...
  // Backfill: any lead with utm_medium=guille and no setter → assign Guille
  const { data: leads } = await sb
    .from("leads")
    .select("id, utm_medium, setter_id")
    .ilike("utm_medium", "guille");
  let backfilled = 0;
  ...
  return NextResponse.json({ ok: true, guille_id: guilleId, campaigns_created: campsCreated, backfilled_leads: backfilled });
}
