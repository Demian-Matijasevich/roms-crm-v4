...
const transferSchema = z
  .object({
    desde: socioEnum,
    hacia: socioEnum,
    // Límite superior razonable para plata en un solo movimiento
    // (evita insertar Number.MAX_SAFE_INTEGER y romper agregaciones).
    ...
    // Formato + rango real. `.length(10)` aceptaba "9999-99-99".
    fecha: z
      .string()
      ...
      .optional(),
    // Mes al que se devenga el gasto (YYYY-MM). Opcional: si no viene,
    // se deriva de `fecha`. NUNCA se infiere del concepto/notas.
    devengado_mes: z
      .string()
  ...
  .refine((d) => d.desde !== d.hacia, {
    message: "desde y hacia deben ser socios distintos",
    path: ["hacia"],
  })
...
export async function POST(req: NextRequest) {
  ...
  const rl = checkRateLimit(`caja-transferencia:${sessionId}`);
  if (!rl.ok) {
  ...
  const idempotencyKey =
    idemHeader && idemHeader.length > 0 && idemHeader.length <= 200
      ? idemHeader
      : autoIdempotencyKey("caja-transferencia", sessionId);
  ...
  const { desde, hacia, monto_usd, monto_ars, fecha, notas, devengado_mes } = parsed.data;
  ...
  try {
    ...
    const desdeNombre = nombreSocio(desde);
    const haciaNombre = nombreSocio(hacia);
    const conceptoBase = `Transferencia socios: ${desdeNombre} → ${haciaNombre}`;
    const concepto = notas ? `${conceptoBase} — ${notas}` : conceptoBase;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        fecha: fechaFinal,
        concepto,
        categoria: "reparto socios",
        monto_usd: mUsd,
        monto_ars: mArs,
        billetera,
        pagado_a: `${haciaNombre} (reparto socios)`,
        pagado_por: desdeNombre,
        estado: "pagado",
        nicho: "general",
    ...
    if (error) {
      ...
      console.error("[POST /api/caja/transferencia] pg error", error);
      return NextResponse.json({ error: "Error al registrar" }, { status: 500 });
    }

    return NextResponse.json({ ok: true, id: data?.id });
  } catch (err) {
    console.error("[POST /api/caja/transferencia]", err);
    return NextResponse.json({ error: "Error al registrar" }, { status: 500 });
  }
}
